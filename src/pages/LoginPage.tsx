...
import { auth, signInWithGoogle, claimNick, checkNickTaken, getNickForUser, getPlayerWins } from '../firebase'
  ...
  useEffect(() => {
    return auth.onAuthStateChanged(async u => {
      ...
      if (claimed) {
        ...
        const w = await getPlayerWins(claimed)
        setWins(w)
      ...
      <Box component="form" onSubmit={handleClaimNick} sx={{ display: 'flex', flexDirection: 'column', gap: 1.25 }}>
        <Box sx={{ display: 'flex', gap: 0.75 }}>
          ...
          <Tooltip title="Suggest a nickname">
            <IconButton onClick={suggest} sx={{ border: '1px solid rgba(255,255,255,0.12)', borderRadius: 1 }}>
